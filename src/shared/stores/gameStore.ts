...
const initialChecklists: Omit<OnboardingChecklist, 'id'>[] = [
  {
    ...
    description:
      'Track your performance in your first game to start building your stats',
    completed: false,
    xpReward: 50,
    icon: '🥍',
    action: 'LOG_GAME',
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  logGame: async gameData => {
    try {
      ...
      if (games.length === 1) {
        const logGameChecklist = onboardingChecklists.find(
          c => c.action === 'LOG_GAME',
        );
    ...
    try {
      ...
      set(state => ({
        games: state.games.map(game =>
          game.id === gameId ? { ...game, stats, updatedAt: new Date() } : game,
        ),
        isLoading: false,
      }));
    } catch (error: any) {
  ...
  initializeChecklists: userId => {
    const checklists: OnboardingChecklist[] = initialChecklists.map(
      (item, index) => ({
        ...item,
  ...
  getSeasonStats: seasonType => {
    ...
    return filteredGames.reduce(
      (totals, game) => ({
        ...
        saves: totals.saves
          ? totals.saves + (game.stats.saves || 0)
          : game.stats.saves || 0,
        goalsAgainst: totals.goalsAgainst
          ? totals.goalsAgainst + (game.stats.goalsAgainst || 0)
          : game.stats.goalsAgainst || 0,
        faceoffWins: totals.faceoffWins
          ? totals.faceoffWins + (game.stats.faceoffWins || 0)
          : game.stats.faceoffWins || 0,
        faceoffLosses: totals.faceoffLosses
          ? totals.faceoffLosses + (game.stats.faceoffLosses || 0)
          : game.stats.faceoffLosses || 0,
      }),
      {
        goals: 0,
        assists: 0,
